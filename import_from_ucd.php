...
function check_existing_item($id,$idtype, $userid) {
  ...
  $q="select itemidentifiervalue from voxb_items where objectid in (select objectid from voxb_objects where objectidentifiervalue=:objectidentifiervalue AND objectidentifiertype=:objectidentifiertype) AND userid=:userid";
  $oci_voxb->set_query($q);
